...
function windowScroll(e) {
  ...
  if (app.settings.isDesktop) {
    ...
    app.els.$shantelle.css("transform", "translateY(" + -t / 5 + "px) rotate(-9deg)")
  }
...
function onCategoryClick(t) {
  closeQuickJump();
  var n = $(t.currentTarget).index();
  $(".ProjectsSidebar").attr("data-active", n),
...
$(document).ready(function (event) {
  ...
  $(".ProjectsSidebarPoint").click(function (event) {
...
function handleParalax() {
  ...
  // this.recalcElementsPos.__throttled(), 
  if (!app.settings.isMobile && this.showItemsOnScroll)
  ...
  let categoriesPos = [];
  $(".ProjectsList .Category").each(function (n, s) {
    categoriesPos.push($(s).offset().top)
  });


  if (!app.settings.isMobile && !this.skipCategoryChangeOnScroll)
    for (var s = categoriesPos.length - 1; s >= 0; s--)
      if (categoriesPos[s] - 60 < t + 1) {
